...
(function () {
  var selectedSlot = '';
  var morningSlots = ['10:00 AM','10:30 AM','11:00 AM','11:30 AM','12:00 PM','12:30 PM'];
  var eveningSlots = ['5:30 PM','6:00 PM','6:30 PM','7:00 PM','7:30 PM','8:00 PM','8:30 PM'];
  var weekendSlots = ['10:00 AM','10:30 AM','11:00 AM','11:30 AM','12:00 PM','12:30 PM'];
  ...
  function buildSlots(containerId, times, dateStr) {
    ...
    times.forEach(function (t) {
      ...
      var parts = t.match(/(\d+):(\d+)\s(AM|PM)/);
      var h = parseInt(parts[1]), min = parseInt(parts[2]), ampm = parts[3];
      if (ampm === 'PM' && h !== 12) h += 12;
      if (ampm === 'AM' && h === 12) h = 0;
      slotDate.setHours(h, min, 0, 0);
      if (dateStr === yyyy + '-' + mm + '-' + dd && slotDate <= now) {
      ...
      btn.addEventListener('click', function () {
        ...
        selectedSlot = t;
        document.getElementById('err-slot').style.display = 'none';
      });
  ...
  function updateSlots() {
    ...
    document.getElementById('morning-label').style.display = 'block';
    document.querySelector('.slot-pick-hint').style.display = 'none';
    buildSlots('morning-slots', morningSlots, date);

    if (isWeekend) {
      document.getElementById('evening-label').style.display = 'none';
      document.getElementById('evening-slots').innerHTML = '';
      document.getElementById('slot-note').textContent = 'Weekend hours: 10:00 AM – 1:00 PM only';
    } else {
      document.getElementById('evening-label').style.display = 'block';
      buildSlots('evening-slots', eveningSlots, date);
      document.getElementById('slot-note').textContent = 'Weekday hours: 10:00 AM – 1:00 PM & 5:30 PM – 9:00 PM';
    }
  }
...
document.getElementById('bk-submit').addEventListener('click', function () {
    ...
    var treatment = document.getElementById('bk-treatment').value;
    var valid = true;
    ...
    if (!date) { document.getElementById('err-date').style.display = 'block'; valid = false; }
    else { document.getElementById('err-date').style.display = 'none'; }

    if (!treatment) { document.getElementById('err-treatment').style.display = 'block'; valid = false; }
    else { document.getElementById('err-treatment').style.display = 'none'; }

    if (!selectedSlot) { document.getElementById('err-slot').style.display = 'block'; valid = false; }
    else { document.getElementById('err-slot').style.display = 'none'; }
    ...
    var message =
      '*Appointment Request — Brite Smile Dental Care*\n\n' +
      '*Name:* ' + name + '\n' +
      '*Date:* ' + formatted + '\n' +
